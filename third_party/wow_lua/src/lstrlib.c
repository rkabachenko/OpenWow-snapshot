...
static int str_dump (lua_State *L) {
  ...
  if (lua_dump(L, writer, &b) != 0) {
    lua_pushliteral(L, "unable to dump given function");
    return lua_error(L);
  }
  luaL_pushresult(&b);
  return 1;
  ...
  while (strfrmt < strfrmt_end) {
    if (*strfrmt != L_ESC)
      ...
      if (isdigit(uchar(*strfrmt))) {
        const size_t remaining = (size_t)(strfrmt_end - strfrmt);
        if (remaining >= 2 && strfrmt[1] == '$') {
          arg = strfrmt[0] - '0';
          strfrmt += 2;
        }
        else if (remaining >= 3 && isdigit(uchar(strfrmt[1])) &&
                 strfrmt[2] == '$') {
