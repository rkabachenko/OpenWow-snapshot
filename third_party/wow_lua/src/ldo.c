...
static size_t luaW_nosavecontext (lua_State *L) { (void)L; return 0; }
static void luaW_norestorecontext (lua_State *L, size_t token) {
  (void)L; (void)token;
}

size_t (*luaW_savehostcontext) (lua_State *L) = luaW_nosavecontext;
void (*luaW_restorehostcontext) (lua_State *L, size_t token) =
    luaW_norestorecontext;
...
int luaD_pcall (lua_State *L, Pfunc func, void *u,
  ...
  int old_executiontaint = G(L)->executiontaint;
  unsigned int old_tainttracking = G(L)->tainttracking;
  size_t host_context = luaW_savehostcontext(L);
  L->errfunc = ef;
  status = luaD_rawrunprotected(L, func, u);
  if (status != 0) {
    G(L)->executiontaint = old_executiontaint;
    G(L)->tainttracking = old_tainttracking;
    luaW_restorehostcontext(L, host_context);
