...
void luaV_gettable (lua_State *L, const TValue *t, TValue *key, StkId val) {
  ...
  for (loop = 0; loop < MAXTAGLOOP; loop++) {
    ...
    if (ttistable(t)) {
      ...
      if (luaW_isnativefieldkey(key) || luaW_isnativetable(h)) {
    ...
    if (ttistable(t)) {
      ...
      if (!ttisnil(oldval) ||
        ...
        if (luaW_isnativefieldkey(key) || luaW_isnativetable(h)) {
          ...
          setobjraw(L, oldval, val);
          luaW_marknativecontainer(key, val);
        } else {
          setobj2t(L, oldval, val);
          luaW_applywritetaint(L, oldval);
        }
  ...
  for (;;) {
    ...
    switch (GET_OPCODE(i)) {
      ...
      case OP_FORPREP: {
        ...
        luaW_setcurrenttaint(L, ra);
        luaW_setcurrenttaint(L, ra + 1);
        luaW_setcurrenttaint(L, ra + 2);
      ...
      case OP_SETLIST: {
        ...
        for (; n > 0; n--) {
          TValue *slot;
          TValue *val = ra+n;
          slot = luaH_setnum(L, h, last--);
          if (luaW_isnativetable(h)) {
            setobjraw(L, slot, val);
          } else {
            setobj2t(L, slot, val);
