...
bool WmoRenderer::UploadGroupIntoSlot(const std::size_t group_index,
  ...
  const auto upload_optional_indices =
      [](const std::vector<std::uint16_t>& indices) {
    return bgfx::createIndexBuffer(bgfx::copy(
        indices.data(), static_cast<std::uint32_t>(
                            indices.size() * sizeof(std::uint16_t))));
  };
  if (!mesh.collision_indices.empty()) {
    gpu.collision_ib = upload_optional_indices(mesh.collision_indices);
    gpu.collision_valid = bgfx::isValid(gpu.collision_ib);
  }
  if (!mesh.outdoor_surface_indices.empty()) {
    gpu.outdoor_surface_ib =
        upload_optional_indices(mesh.outdoor_surface_indices);
    gpu.outdoor_surface_valid = bgfx::isValid(gpu.outdoor_surface_ib);
  }
  if (!mesh.primary_debug_indices.empty()) {
    gpu.primary_debug_ib = upload_optional_indices(mesh.primary_debug_indices);
    gpu.primary_debug_valid = bgfx::isValid(gpu.primary_debug_ib);
  }
  if (!mesh.renderable_indices.empty()) {
    gpu.renderable_ib = upload_optional_indices(mesh.renderable_indices);
    gpu.renderable_valid = bgfx::isValid(gpu.renderable_ib);
  }
...
bool WmoRenderer::PublishPreparedMaterialSubset(
  ...
  for (const std::uint32_t material_index : material_indices) {
    ...
    ready = PublishMaterial(root, material_index, false) && ready;
  }
  return ready;
}
...
bool WmoRenderer::PublishMaterial(
  ...
  const bool diffuse_ready = bgfx::isValid(gpu_mat.diffuse);
  const bool environment_ready =
      !needs_environment || bgfx::isValid(gpu_mat.env_map);
  ...
  const auto flush_run = [&]() {
    ...
    if (transition_batch) {
      ...
      state = (state & ~BGFX_STATE_DEPTH_TEST_MASK) |
              kWmoTransitionExteriorDepthTestState;
    } else if (mat) {
