...
struct M2ParticleLighting {

  const M2BatchUniforms *uniforms = nullptr;

  bx::Vec3 world_normal{0.0f, 0.0f, 1.0f};
};

inline constexpr std::uint32_t kM2ParticleFlagEmitterLocalSpace = 0x00000010u;

inline constexpr std::uint32_t kM2ParticleFlagUnlit = 0x00000001u;
...
struct M2ParticleDrawRange {
  ...
  std::uint8_t material_blend_index = 0;
...
public:
  ...
  const std::vector<ParticleVertex> &BuildVertices(
      const bx::Vec3 &cam_right, const bx::Vec3 &cam_up,
      const M2ParticleLighting *lighting = nullptr);
...
private:
  ...
  bool vertices_lighting_enabled_{false};
  std::uint64_t vertices_lighting_hash_{0u};
...
[[nodiscard]] std::uint8_t ResolveM2ParticleRenderTypeFromBlendMode(std::uint8_t blending_type);
[[nodiscard]] std::uint8_t
ResolveM2ParticleRetailSortBlendIndex(std::uint8_t render_particle_type,
                                      bool force_additive_sort);
[[nodiscard]] bool IsM2ParticleAdditiveSortBlendIndex(std::uint8_t sort_blend_index) noexcept;
