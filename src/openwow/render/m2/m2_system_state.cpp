...
M2ModelInstanceLoadResult M2SystemState::LoadModelInstance(
  ...
  const auto created = instances.Create(loaded.model_id);
  ...
  jobs->ParallelFor(
      ...
      [this, &camera_right, &camera_up](const std::size_t begin,
        ...
        for (std::size_t index = begin; index < end; ++index) {
          static_cast<void>(particle_warm_scratch_[index]
                                ->particle_system.BuildVertices(camera_right,
                                                                camera_up));
        }
      });
}
