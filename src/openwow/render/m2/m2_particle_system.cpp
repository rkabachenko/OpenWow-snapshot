...
bool ShouldBatchWith(const M2ParticleDrawRange &range, const M2ParticleDrawInput &input,
                     ...
                     const std::uint32_t flag_sort_key, const std::uint32_t sort_group) {
  if (!batch_particles) {
    return false;
  }
  ...
  if (range.sort_group != sort_group) {
    return false;
  }
...
std::uint8_t ResolveM2ParticleRenderTypeFromBlendMode(const std::uint8_t blending_type) {
...
bool IsM2ParticleAdditiveSortBlendIndex(const std::uint8_t sort_blend_index) noexcept {

  return sort_blend_index == 3u || sort_blend_index == 4u;
}
...
std::uint32_t ResolveM2ParticleRetailFlagSortKey(const std::uint32_t flags,
  ...
  const bool opaque_like = blending_type == 0u || blending_type == 1u || blending_type > 6u;
...
BuildM2ParticleDrawRanges(const std::vector<M2ParticleDrawInput> &inputs,
  ...
  std::uint32_t sort_group = 0u;
  bool previous_additive = false;
  for (const auto &input : inputs) {
    if (input.vertex_count == 0u) {
      continue;
    }

    const std::uint8_t render_particle_type =
        ResolveM2ParticleRenderTypeFromBlendMode(input.blending_type);

    const std::uint8_t material_blend_index =
        ResolveM2ParticleRetailSortBlendIndex(render_particle_type,
                                              false);
    ...
    const bool additive = IsM2ParticleAdditiveSortBlendIndex(sort_blend_index);
    sort_group += (additive && previous_additive) ? 0u : 1u;
    ...
    ranges.push_back({
        ...
        .material_blend_index = material_blend_index,
  ...
  std::stable_sort(ranges.begin(), ranges.end(),
                   [](const M2ParticleDrawRange &lhs, const M2ParticleDrawRange &rhs) {
                     if (lhs.sort_group != rhs.sort_group) {
                       return lhs.sort_group < rhs.sort_group;
                     }
                     if (lhs.render_particle_type != rhs.render_particle_type) {
                       return lhs.render_particle_type < rhs.render_particle_type;
                     }
...
const std::vector<ParticleVertex> &M2ParticleSystem::BuildVertices(
    const bx::Vec3 &cam_right, const bx::Vec3 &cam_up,
    const M2ParticleLighting *const lighting) {
  ...
  const bool lighting_enabled =
      lighting != nullptr && lighting->uniforms != nullptr;
  std::uint64_t lighting_hash = 0u;
  if (lighting_enabled) {
    constexpr std::uint64_t kFnvOffsetBasis = 1469598103934665603ull;
    constexpr std::uint64_t kFnvPrime = 1099511628211ull;
    lighting_hash = kFnvOffsetBasis;
    const auto &u = *lighting->uniforms;
    const auto push = [&](const float value) {
      lighting_hash =
          (lighting_hash ^ std::bit_cast<std::uint32_t>(value)) * kFnvPrime;
    };
    push(lighting->world_normal.x);
    push(lighting->world_normal.y);
    push(lighting->world_normal.z);
    push(u.light_count[0]);
    for (const float channel : u.light_ambient) {
      push(channel);
    }
    for (int index = 0; index < M2BatchUniforms::kMaxM2Lights; ++index) {
      const auto slot = static_cast<std::size_t>(index);
      for (const float channel : u.light_pos_range[slot]) {
        push(channel);
      }
      for (const float channel : u.light_attenuation[slot]) {
        push(channel);
      }
      for (const float channel : u.light_color[slot]) {
        push(channel);
      }
    }
  }
  if (vertices_valid_ && vertices_state_generation_ == state_generation_ &&
      ...
      vertices_lighting_enabled_ == lighting_enabled &&
      vertices_lighting_hash_ == lighting_hash) {
    return vertices_;
  }
  ...
  for (std::size_t ei = 0; ei < pe.size() && ei < emitters_.size(); ++ei) {
    ...
    const bool emitter_lit =
        lighting_enabled && (def.flags & kM2ParticleFlagUnlit) == 0u &&
        def.blending_type < static_cast<std::uint8_t>(M2BlendMode::Count) &&
        BlendModeUsesMaterialLighting(static_cast<M2BlendMode>(def.blending_type));
    ...
    for (const auto &sort_entry : sorted_particles) {
      ...
      auto lit_col3 = col3;
      if (emitter_lit) {
        const auto scale = EvaluateM2SurfaceLighting(
            *lighting->uniforms,
            {lighting->world_normal.x, lighting->world_normal.y,
             lighting->world_normal.z},
      ...
      const std::uint32_t color =
          PackM2ParticleABGR(lit_col3.x, lit_col3.y, lit_col3.z, alpha);
...
std::optional<std::uint64_t> M2ParticleSystem::BlendStateForType(std::uint8_t blending_type) {
  ...
  const auto blend_state = ResolveM2BlendState(static_cast<M2BlendMode>(blending_type));
  if (!blend_state.has_value()) {
    return std::nullopt;
  }
  return *blend_state | BGFX_STATE_MSAA;
}
