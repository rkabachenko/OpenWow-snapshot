...
const M2BatchUniforms kDefaultBatchUniforms{};
...
M2RenderInstanceResult M2EffectRenderer::SubmitParticles(
  ...
  const auto& vertices =
      instance.particle_system.BuildVertices(camera_right, camera_up);
  if (vertices.empty()) {
  ...
  const M2BatchUniforms& uniforms =
      instance_uniforms != nullptr ? *instance_uniforms : kDefaultBatchUniforms;
  std::uint32_t draw_count = 0;
  for (const auto& range : *ranges) {
    constexpr std::uint32_t kUnfogged = 0x2u;
    const auto base_state = M2ParticleSystem::BlendStateForType(range.blending_type);
    if (!base_state.has_value()) {
      return status("m2.particle.material", M2ResultStatus::kUnsupported,
                    M2ResultReason::kUnsupportedBlendMode,
                    "particle blend=" + std::to_string(range.blending_type), draw_count);
    }

    const auto blend_mode_enum = static_cast<M2BlendMode>(range.blending_type);
    const bool render_flag_unfogged = (range.flag_sort_key & kUnfogged) != 0u;
    ...
    const float alpha_ref =
        ResolveM2AlphaRefThreshold(blend_mode_enum, 1.0f).value_or(kParticleDefaultAlphaRef);
    const RenderVec4 texture_flags{
        1.0f, fog_mode == M2MaterialFogMode::Disabled ? 1.0f : 0.0f, alpha_ref, 0.0f};
    ...
    if (submit_trace.has_value()) {
      RecordM2EffectSubmitTrace(
          ...
          static_cast<std::int32_t>((static_cast<std::uint32_t>(range.sort_blend_index) << 8u) |
          ...
          "texture=" + std::to_string(range.texture_index) + ":" +
              ...
              std::to_string(range.blending_type) + " render_type=" +
              std::to_string(range.render_particle_type) + " sort_blend=" +
...
M2RenderInstanceResult M2EffectRenderer::SubmitRibbons(
  ...
  for (std::size_t index = 0; index < emitters.size() && index < ribbons.size(); ++index) {
    ...
    for (const auto& pass : *passes) {
      ...
      const float ribbon_alpha_ref =
          ResolveM2AlphaRefThreshold(pass.blend_mode, 1.0f).value_or(kRibbonDefaultAlphaRef);
      const RenderVec4 texture_flags{
          1.0f, ribbon_fog_mode == M2MaterialFogMode::Disabled ? 1.0f : 0.0f,
          ribbon_alpha_ref, 0.0f};
