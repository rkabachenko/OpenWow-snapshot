...
void ParticleSystem::Render(uint8_t view_id, const float* view_mtx,
  ...
  for (const auto& batch : batches) {
    ...
    constexpr float kSpellParticleAlphaRef = 1.0f / 255.0f;
    const float particle_flags[4] = {1.0f, 0.0f, kSpellParticleAlphaRef, 1.0f};
    bgfx::setUniform(u_particle_flags_, particle_flags);
...
void ParticleSystem::Shutdown() {
  ...
  if (bgfx::isValid(u_particle_flags_)) bgfx::destroy(u_particle_flags_);
