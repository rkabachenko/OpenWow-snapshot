...
class WmaDecoder final : public IAudioDecoder {
public:
    WmaDecoder();
    ~WmaDecoder() override;

    WmaDecoder(const WmaDecoder&) = delete;
    WmaDecoder& operator=(const WmaDecoder&) = delete;
    WmaDecoder(WmaDecoder&&) noexcept;
    WmaDecoder& operator=(WmaDecoder&&) noexcept;
    ...
    [[nodiscard]] AudioFormat GetFormat() const override { return AudioFormat::WMA; }
