...
const openwow::data::dbc::ChatChannelsEntry *
LookupChatChannelDefinitionByBaseName(const openwow::data::dbc::DbcLoader *dbc,
  ...
  for (const auto& entry : dbc->chat_channels().entries()) {
    ...
    if (openwow::core::SStrCmpUTF8NoCase(entry.name.data(), channel_name.data(),
                                         0x7FFFFFFF) == 0) {
      return &entry;
    }
  }
...
ResolvedJoinChannelRequest ResolveJoinChannelRequest(
  ...
  *valid = true;
  ...
  if (const auto* definition =
          LookupChatChannelDefinitionByBaseName(dbc, requested_name);
      definition != nullptr) {
...
int LuaLeaveChannelByName(lua_State *L) {
  ...
  const std::string channel_name = name != nullptr ? std::string(name) : std::string();

  ::openwow::ui::game::ChatWindowState::Get().RemoveChannelFromAllWindows(channel_name);

  auto &cs = ::openwow::game::ChatSystem::Get();
  const auto *channel = cs.GetChannelByName(channel_name);
  if (channel != nullptr && channel->instance_id == 0) {
    auto pkt = ::openwow::game::ChatManager::BuildLeaveChannel(
        channel->lookup_id, channel->name);
    (void)::openwow::net::ClientServices__SendPacket(pkt);
  }

  cs.LeaveChannel(channel_name);
  return 0;
}
