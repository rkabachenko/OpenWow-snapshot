...
void RefreshZoneSoundsForActiveMover() {
  ...
  const auto &wmo = openwow::world::ActiveMoverWmoSoundContext();

  const auto select_sound = [&wmo](const std::uint32_t wmo_group_value,
                                   const std::uint32_t wmo_root_value,
                                   const std::uint32_t current_area_value,
                                   const std::uint32_t parent_area_value)
      -> std::int32_t {
    if (wmo_group_value != 0u) {
      return static_cast<std::int32_t>(wmo_group_value);
    }
    if (wmo_root_value != 0u) {
      return static_cast<std::int32_t>(wmo_root_value);
    }
    if (wmo.interior_group) {
      return 0;
    }
    if (current_area_value != 0u) {
      return static_cast<std::int32_t>(current_area_value);
    }

    return static_cast<std::int32_t>(parent_area_value);
  };

  const auto area_value = [current_entry](const auto member) {
    return current_entry != nullptr ? current_entry->*member : 0u;
  };

  const std::int32_t sound_provider_pref = select_sound(
      wmo.group.sound_provider_pref, wmo.root.sound_provider_pref,
      area_value(&openwow::data::dbc::AreaTableEntry::sound_provider_pref),
      parent_entry->sound_provider_pref);
  const std::int32_t sound_provider_uw = select_sound(
      wmo.group.sound_provider_pref_uw, wmo.root.sound_provider_pref_uw,
      area_value(&openwow::data::dbc::AreaTableEntry::sound_provider_pref_uw),
      parent_entry->sound_provider_pref_uw);
  const std::int32_t ambience = select_sound(
      wmo.group.sound_ambience_id, wmo.root.sound_ambience_id,
      area_value(&openwow::data::dbc::AreaTableEntry::sound_ambience_id),
      parent_entry->sound_ambience_id);
  const std::int32_t zone_music = select_sound(
      wmo.group.zone_music_id, wmo.root.zone_music_id,
      area_value(&openwow::data::dbc::AreaTableEntry::zone_music_id),
      parent_entry->zone_music_id);
  const std::int32_t intro_music = select_sound(
      wmo.group.zone_intro_music_id, wmo.root.zone_intro_music_id,
      area_value(&openwow::data::dbc::AreaTableEntry::zone_intro_music_id),
  ...
  (void)sound.ResolveWorldStateZoneSoundBinding(
      parent_entry->id, current_area_id, wmo.root.wmo_area_id,
      wmo.group.wmo_area_id, wmo.interior_group,
  ...
  sound.SetSoundProviderSelectionGuards(wmo.interior_group,
                                        sound.GetIndoorSoundAreaId() != 0);
}
