...
void CreateTextureTable(lua_State *L, int parent_idx) {
  ...
  lua_pushcclosure(L, [](lua_State *Ls) -> int {
    ...
    if (lua_isnumber(Ls, 2)) {
      ...
      const auto clamp01 = [](double v) -> double {
        return v < 0.0 ? 0.0 : (v >= 1.0 ? 1.0 : v);
      };
      runtime::SetTextureRenderStateNumber(
          Ls, 1, TextureRenderStateField::kSolidColorR,
          clamp01(lua_tonumber(Ls, 2)));
      runtime::SetTextureRenderStateNumber(
          Ls, 1, TextureRenderStateField::kSolidColorG,
          clamp01(luaL_optnumber(Ls, 3, 0)));
      runtime::SetTextureRenderStateNumber(
          Ls, 1, TextureRenderStateField::kSolidColorB,
          clamp01(luaL_optnumber(Ls, 4, 0)));

      double alpha = 1.0;
      if (lua_isnumber(Ls, 5)) {
        alpha = clamp01(lua_tonumber(Ls, 5));
      }
      runtime::SetTextureRenderStateNumber(
          Ls, 1, TextureRenderStateField::kSolidColorA, alpha);
