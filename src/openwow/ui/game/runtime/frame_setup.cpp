...
void FrameMaterializer::ApplyFullFrameSetup(const UiFrame &frame) {
  ...
  if (!frame.anchors.empty()) {
    lua_newtable(lua_);
    const int anchors = lua_absindex(lua_, -1);
    int item = 1;
    for (const auto &anchor : frame.anchors) {
      lua_newtable(lua_);
      const auto point = framexml::EffectiveAnchorPoint(anchor);
      lua_pushlstring(lua_, point.data(), point.size());
      lua_setfield(lua_, -2, "point");
      if (!anchor.relative_to.empty()) {
        lua_pushstring(lua_, anchor.relative_to.c_str());
        lua_setfield(lua_, -2, "relativeTo");
      }
      const auto relative = framexml::EffectiveAnchorRelativePoint(anchor);
      lua_pushlstring(lua_, relative.data(), relative.size());
      lua_setfield(lua_, -2, "relativePoint");
      lua_pushnumber(lua_, anchor.x);
      lua_setfield(lua_, -2, "x");
      lua_pushnumber(lua_, anchor.y);
      lua_setfield(lua_, -2, "y");
      if (anchor.flags != 0) {
        lua_pushinteger(lua_, anchor.flags);
        lua_setfield(lua_, -2, "__ow_flags");
      }
      lua_rawseti(lua_, anchors, item++);
    }
    lua_setfield(lua_, index, "__ow_anchors");
    openwow::ui::game::detail::ReindexLuaAnchorDependents(lua_, index);
  }
  ApplyAttributes(lua_, index, frame);
}
