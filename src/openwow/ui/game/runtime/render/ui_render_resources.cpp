...
UiRenderResources::AcquireModelSurface(const std::string& key) {
  ...
  model_surface_last_used_[key] = model_surface_pass_;
  return surface;
}

void UiRenderResources::RetireIdleModelSurfaces() {
  ++model_surface_pass_;
  if (model_surface_pass_ < kModelSurfaceIdlePassLimit) {
    return;
  }
  const std::uint64_t cutoff = model_surface_pass_ - kModelSurfaceIdlePassLimit;
  for (auto it = model_surfaces_.begin(); it != model_surfaces_.end();) {
    const auto used = model_surface_last_used_.find(it->first);
    if (used != model_surface_last_used_.end() && used->second > cutoff) {
      ++it;
      continue;
    }

    if (used != model_surface_last_used_.end()) {
      model_surface_last_used_.erase(used);
    }
    it = model_surfaces_.erase(it);
  }
}
