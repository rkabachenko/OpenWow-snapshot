...
void ApplySubjectDisplayNames(QuestDialogTextSubject& subject,
...
QuestDialogTextSubject ResolveQuestDialogTextSubject(
  ...
  ApplySubjectDisplayNames(subject, session.GetDbcLoader());
  return subject;
}
...
bool ExpandServerTextTokens(const ObjectManager& objects,
                            const data::dbc::DbcLoader* dbc_loader,
                            const ObjectGuid subject_guid,
                            const std::string_view raw_text,
                            std::string& expanded) {
  QuestDialogTextSubject subject;

  auto guid = subject_guid;
  if (guid.IsEmpty()) {
    guid = objects.GetActivePlayerGuid();
  }
  ...
  if (!guid.IsEmpty()) {
    ...
    if (const auto* name_entry = objects.GetNameEntry(guid)) {
      subject.has_context = true;
      if (subject.name.empty()) subject.name = name_entry->name;
      if (subject.class_id == 0) subject.class_id = name_entry->class_id;
      if (subject.race_id == 0) subject.race_id = name_entry->race;
      if (subject.gender == 0) subject.gender = name_entry->gender;
    }
    if (subject.name.empty()) {
      subject.name = objects.GetPlayerName(guid);
    }
  }

  ApplySubjectDisplayNames(subject, dbc_loader);
  if (!subject.has_context) {

    return false;
  }
  expanded = ExpandQuestDialogText(subject, raw_text, false);
  return true;
}
