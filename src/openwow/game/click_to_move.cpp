...
void ClickToMoveSystem::AttackUnit(ObjectGuid guid, float x, float y, float z) {
    ...
    SetArrivalThreshold(kCtmAttackArrivalRadius);
}
...
bool ClickToMoveSystem::ReachedDestination(const float player_x,
    ...
    return IsAtDestination(player_x, player_y, player_z, arrival_threshold_);
}
...
void ClickToMoveSystem::Reset() {
    ...
    dest_y_    = 0.0f;
    dest_z_    = 0.0f;
    arrival_threshold_ = kCtmMoveArrivalRadius;
    completed_action_.reset();
    ClearActiveState();
}
