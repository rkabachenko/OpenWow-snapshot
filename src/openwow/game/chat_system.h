...
[[nodiscard]] const data::dbc::ChatChannelsEntry* FindChatChannelDefinitionByRowName(
    const data::dbc::DbcLoader* dbc, const char* name);
...
struct ChatChannel {
  ...
  bool client_requested_leave = false;
  std::uint8_t flags = 0;
...
public:
  ...
  void MarkChannelClientRequestedLeave(const std::string &name);
