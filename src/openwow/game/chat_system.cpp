...
const data::dbc::ChatChannelsEntry* FindChatChannelDefinitionByRowName(
    const data::dbc::DbcLoader* const dbc, const char* const name) {
  if (dbc == nullptr || name == nullptr || name[0] == '\0') {
    return nullptr;
  }
  ...
  for (const auto& entry : dbc->chat_channels().entries()) {
    ...
    if (openwow::core::SStrCmpUTF8NoCase(entry.name.data(), name, 0x7FFFFFFF) == 0) {
      return &entry;
    }
  }
...
void ChatSystem::MarkChannelClientRequestedLeave(const std::string &name) {
  std::lock_guard<std::mutex> lock(mutex_);

  if (auto *channel = FindChannelByNameLocked(name); channel != nullptr) {
    channel->client_requested_leave = true;
  }
}
