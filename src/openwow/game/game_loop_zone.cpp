...
void GameLoop::SynchronizeZoneUiState() {
  ...
  const auto wmo_group_area_id = static_cast<std::int32_t>(wmo.group.wmo_area_id);
  const auto wmo_root_area_id = static_cast<std::int32_t>(wmo.root.wmo_area_id);
  if (map_id == synchronized_zone_map_id_ &&
      zone_id == synchronized_zone_id_ && area_id == synchronized_area_id_ &&
      wmo_group_area_id == synchronized_wmo_group_area_id_ &&
      wmo_root_area_id == synchronized_wmo_root_area_id_ &&
      wmo.interior_group == synchronized_wmo_interior_group_) {
    return;
  }
  ...
  synchronized_wmo_group_area_id_ = wmo_group_area_id;
  synchronized_wmo_root_area_id_ = wmo_root_area_id;
  synchronized_wmo_interior_group_ = wmo.interior_group;
}

void GameLoop::UpdateCameraUnderwaterSoundArea() {

  if (world_session() == nullptr) {
    return;
  }
  const auto &camera_position = world_scene_.camera().frame_pose().position;
  const std::uint32_t camera_liquid_type_id =
      world_scene_.world_map().GetUnderwaterLiquidTypeId(
          camera_position[0], camera_position[1], camera_position[2]);
  sound_runtime_.SetIndoorSoundArea(
      static_cast<std::int32_t>(camera_liquid_type_id));
}
