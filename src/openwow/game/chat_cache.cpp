...
bool SyncBuiltinZoneChannel(WorldSession& session,
                            const data::dbc::DbcLoader* const dbc,
                            const std::uint32_t current_area_id,
                            const data::dbc::ChatChannelsEntry& definition) {
  if (!IsAvailableZoneChannelDefinition(dbc, current_area_id, definition)) {
    return false;
  }

  return QueueAndSendBuiltinChannelJoin(session, definition);
}
...
bool LeaveJoinedChannel(WorldSession& session, const ChatChannel& channel) {
  ...
  session.interaction().SendLeaveChannel(channel.lookup_id, channel.name);
  ChatSystem::Get().MarkChannelClientRequestedLeave(channel.name);
  return true;
}

std::uint32_t ValidZoneChannelMaskBits(const data::dbc::DbcLoader* const dbc) {
  if (dbc == nullptr) {
    return 0;
  }
  ...
  std::uint32_t bits = 0;
  for (const auto& entry : dbc->chat_channels().entries()) {
    bits |= ChannelMaskBitFromId(entry.id);
  }
  return bits;
}
...
enum class GuildRecruitmentSyncTrigger {
  kAutoJoinSettingChanged,
  kZoneTick,
};

void SyncGuildRecruitmentChannel(WorldSession& session,
                                 const GuildRecruitmentSyncTrigger trigger) {
  ...
  if (!auto_join) {
    ...
    if (trigger == GuildRecruitmentSyncTrigger::kAutoJoinSettingChanged &&
        existing != nullptr && !state.suppress_recruitment_leave_on_disable) {
...
void SetGlobalZoneChannelBitForChannelId(const std::uint32_t channel_id) {
  const auto bit = ChannelMaskBitFromId(channel_id);
  if (bit == 0) {
    return;
  }
  GetChatCacheRuntimeState().global_zone_mask |= bit;
}
...
void SetGuildRecruitmentChannelAutoJoin(WorldSession& session,
  ...
  if (!value_changed) {
    SyncGuildRecruitmentChannel(session,
                                GuildRecruitmentSyncTrigger::kAutoJoinSettingChanged);
  }
}
...
void ApplyChatCachePayload(WorldSession& session,
  ...
  guild_recruitment_callback_handle = cvars.AddCallback(
      ...
      [&session](const std::string&, const std::string&) {
        SyncGuildRecruitmentChannel(
            session, GuildRecruitmentSyncTrigger::kAutoJoinSettingChanged);
      });
    ...
    if (line.rfind("ZONECHANNELS ", 0) == 0) {
      ...
      if (current_window == -1) {
        ...
        runtime_state.global_zone_mask = static_cast<std::uint32_t>(*mask) &
                                         ValidZoneChannelMaskBits(session.GetDbcLoader());
...
void TrySyncLoadedChatChannels(WorldSession& session) {
  ...
  const bool first_pass = !runtime_state.joined_loaded_channels;
  if (!first_pass) {
    SyncGuildRecruitmentChannel(session, GuildRecruitmentSyncTrigger::kZoneTick);
    return;
  }

  if (runtime_state.use_default_zone_channels) {
    PopulateDefaultZoneChannels(dbc, &runtime_state.global_zone_mask);
    runtime_state.use_default_zone_channels = false;
  }

  ApplyWindowZoneChannels(dbc, runtime_state.window_zone_masks);
  ...
  for (const auto& entry : dbc->chat_channels().entries()) {
    ...
    changed = SyncBuiltinZoneChannel(session, dbc, current_area_id, entry) || changed;
  }
...
std::string SerializeChatCache(const data::dbc::DbcLoader* ,
                               const std::uint32_t ) {
  ...
  for (const auto& channel : channels) {
    ...
    if (channel.is_joined && !channel.lua_hidden && !channel.client_requested_leave) {
      global_zone_mask |= ChannelMaskBitFromId(channel.lookup_id);
    }
  ...
  for (std::size_t window_index = 0; window_index < windows.size(); ++window_index) {
    ...
    for (const auto& slot : window.channels) {
      ...
      window_zone_mask |= ChannelMaskBitFromId(assignment.number);

      const auto matched = std::find_if(
          channels.begin(), channels.end(), [&](const ChatChannel& channel) {
            return !channel.client_requested_leave &&
                   core::SStrCmpUTF8NoCase(channel.name.c_str(),
                                           assignment.name.c_str(),
                                           0x7FFFFFFFu) == 0;
          });
      if (matched != channels.end()) {
        out << assignment.name << "\n";
      }
    }
    out << "END\n\n";
