...
WorldMap::ResolveAreaEnvironmentAtPosition(
  ...
  const auto lookup_wmo_area = [&](const std::uint32_t wmo_id,
                                   const std::uint32_t name_set_id,
                                   const std::int32_t group_id)
      -> std::optional<WmoAreaLookupValue> {
    if (const auto *exact = openwow::data::DBClient_FindWmoAreaTable(
            dbc_, wmo_id, name_set_id, group_id);
        exact != nullptr) {
      return WmoAreaLookupValue{exact->area_table_id, exact->flags};
    }
    if (const auto *fallback = openwow::data::DBClient_FindWmoAreaTable(
            dbc_, wmo_id, name_set_id, -1);
        fallback != nullptr) {
      return WmoAreaLookupValue{fallback->area_table_id, fallback->flags};
    }
    return std::nullopt;
  };
  ...
  const auto inspect_group = [&](const WmoAreaGroupRef &ref,
    ...
    if (const auto correction = CorrectWmoRoomHitThroughPortal(
      ...
      group_primary.room = correction->room;
    }
    if (group_primary.room.has_value()) {
      AccumulateWmoRoomHit(lane.primary, *group_primary.room,
                           group_primary.fraction);
    }
    if (group_secondary.room.has_value()) {
  ...
  if (primary_lane.primary.room.has_value()) {
    ...
    const auto instance_it = wmo_instances_.find(best_group->placement);
    if (instance_it != wmo_instances_.end()) {
      const WmoInstance &instance = instance_it->second;
      const auto cache_it = wmo_cache_.find(instance.wmo_path);
      if (cache_it != wmo_cache_.end() &&
          best_group->group_index < cache_it->second.groups.size() &&
          best_group->group_index < cache_it->second.root.groupInfos.size()) {
        const CachedWmo &cached = cache_it->second;
        const std::size_t group_index = best_group->group_index;
        ...
        const std::uint32_t group_flags =
            group_resident ? header.flags
                           : cached.root.groupInfos[group_index].flags;
        const auto wmo_area = lookup_wmo_area(
            cached.root_wmo_id, instance.name_set,
            group_resident ? static_cast<std::int32_t>(header.wmoGroupID)
                           : -1);
        best_area_id = wmo_area.has_value() ? wmo_area->area_id : 0u;

        best_outdoors = (group_flags & data::wmo::kMogpExterior) != 0u;
        if (wmo_area.has_value()) {
          constexpr std::uint32_t kWmoAreaFlagForceOutdoor = 0x4u;
          constexpr std::uint32_t kWmoAreaFlagForceIndoor = 0x2u;
          if ((wmo_area->flags & kWmoAreaFlagForceOutdoor) != 0u) {
            best_outdoors = true;
          } else if ((wmo_area->flags & kWmoAreaFlagForceIndoor) != 0u) {
            best_outdoors = false;
          }
        }
      }
