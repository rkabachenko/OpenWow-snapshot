...
inline void AccumulateWmoRoomHit(WmoRoomHit &hit,
...
inline void NormalizeRetailWmoRoomHits(
  ...
  for (WmoRoomHitLane &slot : placement_slots) {
    if (!slot.primary.room.has_value() && slot.secondary.room.has_value()) {
      slot.primary = slot.secondary;
    }
  }
  if (!placement_slots[0].primary.room.has_value() &&
      placement_slots[1].primary.room.has_value()) {
    placement_slots[0] = placement_slots[1];
    placement_slots[1] = {};
  }
  for (WmoRoomHitLane &slot : placement_slots) {
    if (!slot.secondary.room.has_value() && slot.primary.room.has_value()) {
...
CorrectWmoRoomHitThroughPortal(
  ...
  for (std::size_t ref_index = ref_begin; ref_index < ref_end; ++ref_index) {
    ...
    correction = WmoPortalRoomCorrection{
        .room = {.placement = source_room.placement,
                 .group_index = portal_ref.groupIndex},
        .fraction = fraction};
  }
  return correction;
}
